...
logging2.moderation.mute_role = muteRole.id
    ...
    if(textChats){
      ...
      .setColor('PURPLE')
      .setDescription(`Hey Poggers! I'm **a Bot**.\n\nThank you for inviting me to your server as it means a lot to us! You can get started with [\`s!help\`](https://swordie.swbotdc.repl.co) & customise your server settings by accessing the Dashboard [\`here\`](https://swordie.swbotdc.repl.co/dashboard/${guild.id}).\n\n__**Current News**__\n\`\`\`\nSwordie is currently heavy in development. contact the owner for a free premium key!\`\`\`\n\nAgain, thank you for inviting me! (this server is now very pog)\n****`)
    ...
    .setColor(`PURPLE`)
    .setTitle('New Server')
    .setThumbnail(`https://swordie.swbotdc.repl.co/logo.png`)
    .setDescription(`${jsconfig.bot_name} was added to a new Server!`)
    .addField(`Server Name`, `\`${guild.name}\``, true)
    .addField(`Server ID`, `\`${guild.id}\``, true)
    .setFooter(`${this.client.guilds.cache.size} guilds `,  `https://swordie.swbotdc.repl.co/logo.png`);
