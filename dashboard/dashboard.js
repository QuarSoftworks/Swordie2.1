...
const premiumWeb = new Discord.WebhookClient("981275654072983663", "N_lelyd5hnLrfZLYCYXXmLspIcEjNgy7rbTIg4daRAGe4ujkMQb2XXtFgTQHj9w2Ggcc");
...
const secret = config.secret;
  ...
  module.exports = async (client) => {
    ...
    passport.use(new Strategy({
      clientID: `${clientID}`,
      clientSecret: `${secret}`,
      callbackURL: `https://swordie.swbotdc.repl.co/callback`,
  ...
  store :MongoStore.create({ mongoUrl: jsonconfig.mongodb_url})
    ...
    const renderTemplate = (res, req, template, data = {}) => {
      ...
      const baseData = {
        ...
        url: res,
        req: req,
        image: `https://swordie.swbotdc.repl.co/logo.png`,
    ...
    app.get("/callback", passport.authenticate("discord", {
      ...
      const loginLogs = new WebhookClient('981260798070513746', "gjWaeQ681cM5MoPHlx1wAqekVh9AQGzH47_ZCP3s5ImCKOfYXGG7h7Woc7IAQmlf_jsZ");
      ...
      try {
        ...
        if (req.session.backURL) {
          ...
          if (member) {
            ...
            loginLogs.send({
              username: 'Login Logs',
              avatarURL: `https://swordie.swbotdc.repl.co/logo.png`,
              embeds: [login]
            });
          }
...
var url = req.protocol + '://' + req.get('host') + req.originalUrl;
    ...
    app.get("/support", (req, res) => {
      res.redirect(`https://discord.gg/az5FHG6AHA`)
    });
...
res.redirect(`https://discord.com/api/oauth2/authorize?client_id=969249154406699099&permissions=8&redirect_uri=https://swordie.swbotdc.repl.co&response_type=code&scope=bot%20identify`)
});
    ...
    app.get("/logout", async function(req, res) {
      ...
      const member = await client.users.fetch(req.user.id);
        if (member) {
          ...
          const logoutLogs = new Discord.WebhookClient("981260798070513746", "gjWaeQ681cM5MoPHlx1wAqekVh9AQGzH47_ZCP3s5ImCKOfYXGG7h7Woc7IAQmlf_jsZ");
...
let embed;
  ...
  if(data[i + 1]){
    ...
    .setDescription(`**Link:** [https://swordie.swbotdc.repl.co/paste/${ticketID}](https://swordie.swbotdc.repl.co/paste/${ticketID})\n\n[or click here](https://swordie.swbotdc.repl.co/paste/${ticketID})\n\n**Form ID**: \`${ticketID}\`\n\n**Submitted by:** ${member} **(${member.user.tag} - ${member.id})**\n**Time:** ${moment(new Date()).format("dddd, MMMM Do YYYY HH:mm:ss")}`)
    .setFooter('')
    .setColor('GREEN')
...
if(results.length >= 10) return;
    ...
    app.post("/report", async (req, res) => {
      ...
      if (req.body.type === "report") {
        const reportEmbed = new Discord.WebhookClient('981268308156379206', 'JsHTf8Fh7AJq_AaL4G6IlbdNQv5VDyPapmoZoriVOg5f3QtUbE6-wSm1t0PCG0R01EDv');
    ...
    app.post("/contact", async (req, res) => {
      ...
      if (req.body.type === 'contact') {
        ...
        const contactEmbed = new Discord.WebhookClient('981268308156379206', 'JsHTf8Fh7AJq_AaL4G6IlbdNQv5VDyPapmoZoriVOg5f3QtUbE6-wSm1t0PCG0R01EDv');
      ...
      if (Object.prototype.hasOwnProperty.call(data, "sendEmbed")) {
        ...
        if (ticketSettings.ticketToggle == "true") {
          ...
          if (ticketSettings.ticketType == "reaction" && ticketSettings.ticketCustom == "false") {
            ...
            let reactionFooter = "Powered by Swordie Premium";



            let footer = "Powered by Swordie Premium";
            ...
            if (storedSettings.isPremium == "false") {
              ticketEmbed.setFooter(`Powered by swordie.swbotdc.repl.co`)
            } else {
...
if(maintenance && maintenance.toggle == "true") {
    ...
    app.post('/dblwebhook', webhook.middleware(), async (req) => {
      ...
      const msg = new Discord.MessageEmbed()
        .setAuthor('Voting System', `https://swordie.swbotdc.repl.co/logo.png`)
        .setColor('#7289DA')
...
let voteUser = await client.users.fetch(apiUser.id);
    ...
    app.get("*", (req, res) => {
      ...
      if (fullUrl == domain || fullUrl == `https://swordie.swbotdc.repl.co/style.css` || fullUrl == `https://swordie.swbotdc.repl.co/style.css` || fullUrl == `https://swordie.swbotdc.repl.co/favico.ico` ) {
